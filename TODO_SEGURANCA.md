...
- ~~`authFetch` usa `window.alert` para sessão expirada → trocar por toast/redirect silencioso.~~ ✅ (2026-07-30)
  - O `alert` era bloqueante **de propósito**: `authFetch` faz `window.location.href = '/'` logo depois, e um toast montado antes do reload morreria junto. Agora o aviso é enfileirado em `sessionStorage` (`core/utils/sessionNotice.ts`) e consumido depois do redirect pelo `<SessionNotice />`, montado na raiz do app.
