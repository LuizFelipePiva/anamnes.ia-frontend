...
from app.models.schemas import (
    ...
    SimuladoPublic,
    SimuladoRecommendationResponse,
    SimuladoReport,
)
...
@router.get("/filter-options", response_model=SimuladoFilterOptions)
def get_filter_options(
    _user: dict = Depends(_current_user),
    sb: Client = Depends(get_supabase_client),
):
    """Retorna os valores distintos de banca, tema, etc. para popular os filtros."""
    return simulado_service.get_filter_options(sb)


@router.post("/count-available")
def count_available(
    filters: dict,
    _user: dict = Depends(_current_user),
    sb: Client = Depends(get_supabase_client),
):
    """Conta quantas questões no banco batem com os filtros fornecidos."""
    count = simulado_service.count_available_questions(sb, filters)
    return {"count": count}
...
def recommended_simulados(
    ...
    """Recomenda simulados já acessíveis para a especialidade informada."""
