...
# =============================================
# EXAMES COMPLEMENTARES ANEXADOS AO CASO (SPEC-014)
# =============================================

def _assert_case_owner(case_id: str, payload) -> dict:
    """404 se o caso não existe, 403 se não é do professor. Admin sempre passa."""
    teacher_id = payload.get("sub")
    role = payload.get("role", "teacher")
    sb = get_supabase_client()

    case_res = sb.table("cases").select("id, teacher_id").eq("id", case_id).execute()
    if not case_res.data:
        raise http_error(404, "case_not_found", "Caso não encontrado")
    if role != "admin" and case_res.data[0]["teacher_id"] != teacher_id:
        raise http_error(403, "access_denied", "Acesso negado")
    return case_res.data[0]


@router.get("/{case_id}/exams", response_model=list[MedicalAssetTeacher])
def list_case_exams(case_id: str, payload=Depends(verify_teacher)):
    """Exames anexados ao caso, na visão do professor (com diagnóstico)."""
    validate_uuid(case_id, "case_id")
    _assert_case_owner(case_id, payload)
    sb = get_supabase_client()
    ...
    assets.sort(key=lambda a: a.get("position") or 0)
    return [to_teacher_dto(a, SUPABASE_URL) for a in assets]


@router.put("/{case_id}/exams", response_model=list[MedicalAssetTeacher])
def set_case_exams(case_id: str, data: CaseExamsUpdate, payload=Depends(verify_teacher)):
    """Substitui o conjunto de exames anexados ao caso.

    Substituição (delete + insert) em vez de diff incremental: o cliente manda
    o estado desejado inteiro, então a operação é idempotente e reenviar a mesma
    lista não duplica nada — o que também dispensa endpoints separados de
    anexar/desanexar. Lista vazia desanexa tudo.
    """
    validate_uuid(case_id, "case_id")
    _assert_case_owner(case_id, payload)
    sb = get_supabase_client()

    asset_ids = normalize_asset_ids(data.asset_ids)

    if len(asset_ids) > MAX_EXAMS_PER_CASE:
        raise http_error(
            422, "too_many_exams",
            f"Máximo de {MAX_EXAMS_PER_CASE} exames por caso",
            limit=MAX_EXAMS_PER_CASE,
        )

    # Valida antes de apagar: um ID inválido não pode deixar o caso sem os
    # exames que já tinha.
    if asset_ids:
        found = sb.table("medical_assets") \
            .select("id, attachment_eligible") \
            .in_("id", asset_ids) \
            .execute()
        by_id = {row["id"]: row for row in (found.data or [])}

        missing = [a for a in asset_ids if a not in by_id]
        if missing:
            raise http_error(
                422, "asset_not_found",
                f"Exame não encontrado: {', '.join(missing)}",
                asset_ids=missing,
            )

        blocked = [a for a in asset_ids if not by_id[a].get("attachment_eligible")]
        if blocked:
            raise http_error(
                422, "asset_not_attachable",
                f"Exame não pode ser anexado: {', '.join(blocked)}",
                asset_ids=blocked,
            )

    sb.table("case_exam_assets").delete().eq("case_id", case_id).execute()

    if asset_ids:
        sb.table("case_exam_assets").insert([
            {"case_id": case_id, "asset_id": asset_id, "position": index}
            for index, asset_id in enumerate(asset_ids)
        ]).execute()

    logger.info(f"[Cases] Caso {case_id}: {len(asset_ids)} exame(s) anexado(s)")
    return list_case_exams(case_id, payload)
...
def start_attempt(case_id: str, payload=Depends(verify_jwt_token)):
    ...
    return CaseStartResponse(
        ...
        physical_exam=(case_data.get("form_data") or {}).get("exame_fisico"),
    )
...
def complete_attempt(case_id: str, data: CaseCompleteRequest, payload=Depends(verify_jwt_token)):
    ...
    sb.table("case_attempts").update({
        ...
        "status": "completed",
        "feedback": feedback,
        # SPEC-013 §6.8: persiste a nota por dimensão S/O/A/P para o perfil SOAP.
        # `None` quando a IA falhou ou devolveu formato antigo — a coluna é nullable.
        "breakdown": breakdown,
