...
from app.config import WEEKLY_SUMMARY_ENABLED, logger
...
# SPEC-013 §6.8 — perfil SOAP. Dimensões na ordem canônica S/O/A/P, iguais às
# chaves gravadas por `eval_service.evaluate_soap`.
_SOAP_DIMS = ("subjetivo", "objetivo", "avaliacao", "plano")

# Janela móvel: só as N tentativas avaliadas mais recentes entram na média, para
# o card refletir o desempenho atual do aluno e não a média histórica achatada.
_SOAP_WINDOW = 10


def _soap_profile(attempts: list[dict]) -> dict | None:
    """
    Média por dimensão S/O/A/P nas últimas `_SOAP_WINDOW` tentativas avaliadas.

    `attempts` já vem ordenado por `started_at` desc. Tentativas anteriores à
    migration do `breakdown` (e as em que a IA falhou) não têm o campo e são
    ignoradas. Devolve `None` quando nenhuma tentativa tem breakdown — o
    mínimo de tentativas para exibir o card é regra de UI, aplicada no front.
    """
    window = []
    for a in attempts:
        bd = a.get("breakdown")
        ...
        # Breakdown parcial (formato antigo/truncado) não entra: distorceria a
        # média de uma dimensão em relação às outras.
        if len(scores) == len(_SOAP_DIMS):
            window.append(scores)
        if len(window) == _SOAP_WINDOW:
            break
    ...
    return {
        "attempts": len(window),
        **{
            dim: round(sum(s[dim] for s in window) / len(window), 1)
            for dim in _SOAP_DIMS
        },
    }
...
def _build_profile(student_id: str, sb):
    ...
    attempts_res = sb.table("case_attempts") \
        .select("id, case_id, conversation_id, status, score, feedback, breakdown, started_at, duration_seconds, cases(title, specialty, difficulty)") \
        .eq("student_id", student_id) \
    ...
    return {
        ...
        "soap_profile":  _soap_profile(attempts),
...
def get_my_weekly_summary(request: Request, payload=Depends(verify_jwt_token)):
    """
    Resumo de coaching da semana corrente do aluno, gerado por IA.

    Responde 404 com a flag `WEEKLY_SUMMARY_ENABLED` desligada (default) — é a
    trava de custo do backend, independente da flag do front.

    `{"summary": null}` quando o aluno não teve atividade na semana ou a geração
    falhou: o card some, sem erro na home.
    """
    if not WEEKLY_SUMMARY_ENABLED:
        raise http_error(404, "feature_disabled", "Resumo semanal desativado")

    sb = get_supabase_client()
    result = weekly_summary_service.get_weekly_summary(
        sb,
        student_id=payload.get("sub", ""),
        student_name=payload.get("name", ""),
        # Idioma do claim do JWT, como no chat e na avaliação SOAP (SPEC-011).
