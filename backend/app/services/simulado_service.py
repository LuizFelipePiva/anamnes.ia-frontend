...
def _sample_questions(
    ...
    """
    Sorteia `num_questions` questões respeitando os filtros multi-seleção.
    Lista vazia = sem filtro para aquele campo.
    Busca até 500 candidatas para ter margem de aleatoriedade.
    """
    query = sb.table("questions_bank").select("id").limit(500)

    if specialties:
        query = query.in_("specialty", specialties)
    if bancas:
        query = query.in_("banca", bancas)
    if temas:
        query = query.in_("tema", temas)
    if subtemas:
        query = query.in_("subtema", subtemas)
    if anos:
    ...
    n = min(num_questions, len(rows))
    chosen_ids = [r["id"] for r in random.sample(rows, n)]
    return chosen_ids


def _fetch_questions_public(sb: Client, question_ids: list[str]) -> list[dict]:
    """Busca questões pelo id SEM gabarito (correct_answer/explanation excluídos)."""
    resp = (
        sb.table("questions_bank")
        .select("id, statement, options, specialty, subspecialty, banca, tema, subtema, ano, image_url, created_at")
        .in_("id", question_ids)
        .execute()
    )
    rows = resp.data or []
...
def get_filter_options(sb: Client) -> dict:
    """
    Retorna valores distintos e a árvore de hierarquia (temas por especialidade)
    para popular os painéis de filtro no frontend.
    """
    # 1. Buscamos todas as combinações de specialty e tema
    resp_esp_tema = sb.table("questions_bank").select("specialty, tema").not_.is_("specialty", "null").execute()
    rows = resp_esp_tema.data or []
    
    temas_por_especialidade: dict[str, set[str]] = {}
    for r in rows:
        esp = r.get("specialty")
        tema = r.get("tema")
        if esp not in temas_por_especialidade:
            temas_por_especialidade[esp] = set()
        if tema:
            temas_por_especialidade[esp].add(tema)

    # 2. Busca listagens lineares para bancas e subtemas
    def distinct(col: str) -> list[str]:
        resp = sb.table("questions_bank").select(col).not_.is_(col, "null").execute()
        return sorted({r[col] for r in (resp.data or []) if r.get(col)})

    # 3. Busca anos (inteiros)
    def distinct_anos() -> list[int]:
        resp = sb.table("questions_bank").select("ano").not_.is_("ano", "null").execute()
        return sorted({int(r["ano"]) for r in (resp.data or []) if r.get("ano") is not None})

    # Converter set para lista ordenada
    tree_dict = {
        esp: sorted(list(temas))
        for esp, temas in temas_por_especialidade.items()
    }

    return {
        "specialties": sorted(list(temas_por_especialidade.keys())),
        "temas_por_especialidade": tree_dict,
        "bancas": distinct("banca"),
        "subtemas": distinct("subtema"),
        "anos": distinct_anos(),
    }


def count_available_questions(sb: Client, filters: dict) -> int:
    """
    Retorna a quantidade exata de questões disponíveis no banco
    que correspondem aos filtros informados.
    """
    query = sb.table("questions_bank").select("id", count="exact")
    
    specialties = filters.get("specialties") or []
    bancas      = filters.get("bancas") or []
    temas       = filters.get("temas") or []
    subtemas    = filters.get("subtemas") or []
...
def create_simulado(sb: Client, data: dict, created_by: str) -> dict:
    """
    Cria o cabeçalho do simulado e sorteia questões com base nos filtros.
    Retorna o simulado criado (sem as questões).
    """
    num_questions = data.get("num_questions", 40)
    specialties = data.get("specialties") or []
    bancas      = data.get("bancas")      or []
    temas       = data.get("temas")       or []
    subtemas    = data.get("subtemas")    or []
    anos        = data.get("anos")        or []

    # Valida strings de filtro
    for lst in (specialties, bancas, temas, subtemas):
        for v in lst:
            if len(v) > 200:
                raise ValueError("Valor de filtro excede o tamanho permitido")
    ...
    # Sorteia as questões
    question_ids = _sample_questions(sb, num_questions, specialties, bancas, temas, subtemas, anos)
    ...
    payload = {
        ...
        "specialty":     specialties[0] if len(specialties) == 1 else None,
        "subspecialty":  None,
...
def recommend_simulados(
    ...
    """Recomenda simulados acessíveis na especialidade pedida.

    O conjunto candidato vem exclusivamente de ``list_simulados`` para que a
    recomendação herde a mesma regra de autorização da listagem normal.
    """
    now = now or datetime.now(UTC)
    candidates = [
        item
        for item in list_simulados(sb, user_id, role)
    ...
    if role == "student":
        completed = {
            ...
            if row.get("status") == "completed" and row.get("simulado_id")
        }
