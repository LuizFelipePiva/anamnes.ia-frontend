...
PUBLIC_FIELDS = "id, statement, options, specialty, subspecialty, banca, tema, subtema, ano, image_url, created_at"
PUBLIC_FIELDS_BASIC = "id, statement, options, specialty, subspecialty, image_url, created_at"
...
def list_public_questions(sb: Client, limit: int = 50, offset: int = 0) -> list[dict]:
    ...
    Tenta com campos completos; se a tabela ainda não tiver as colunas extras
    (banca/tema/subtema/ano), cai para a query básica automaticamente.
    """
    try:
        resp = (
        ...
        if "does not exist" in str(e):
            # Colunas opcionais ainda não criadas — usa campos básicos
            resp = (
                sb.table("questions_bank")
                .select(PUBLIC_FIELDS_BASIC)
                .order("created_at", desc=True)
                .range(offset, offset + limit - 1)
                .execute()
            )
