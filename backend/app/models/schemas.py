...
class QuestionCreate(BaseModel):
    ...
    banca: str | None = Field(None, max_length=100)
    tema: str | None = Field(None, max_length=200)
    subtema: str | None = Field(None, max_length=200)
...
class SimuladoFilterOptions(BaseModel):
    """Valores distintos disponíveis no banco para popular os filtros do formulário."""
    specialties: list[str]
    temas_por_especialidade: dict[str, list[str]]
    bancas: list[str]
    subtemas: list[str]
    anos: list[int]
...
class SimuladoCreate(BaseModel):
    ...
    # Filtros multi-seleção — lista vazia = sem filtro (todas as opções)
...
class SimuladoRecommendationResponse(BaseModel):
    """Envelope de simulados recomendados para uma especialidade."""
    specialty: str
...
class CaseExamsUpdate(BaseModel):
    """Substitui o conjunto de exames anexados a um caso.

    A ordem da lista vira `position` — é como o professor controla a sequência
    exibida ao aluno. Lista vazia é válida e significa "desanexar tudo".
    """
    asset_ids: list[str] = Field(default_factory=list)


class MedicalAssetTeacher(BaseModel):
    """Item do catálogo na visão do professor — **inclui o gabarito**."""
    id: str
    modality: str | None = None
    display_name: str | None = None
    diagnosis_or_finding: str | None = None
    url: str | None = None
    media_type: str = "image"
    tags: list[str] = Field(default_factory=list)
    attachment_eligible: bool = True
    credits: Any | None = None
    license: str | None = None
    source_url: str | None = None


class ExamLabel(BaseModel):
    """Rótulo neutro: chave i18n + numeração de desempate (SPEC-014 §6)."""
    key: str
    ordinal: int | None = None


class MedicalAssetStudent(BaseModel):
    """Item na visão do aluno.

    Modelo **separado** do de professor, não um subconjunto: campo novo no
    catálogo não vaza para cá por esquecimento.

    Sem `id`, de propósito: os identificadores do acervo são falantes
    (`RX-PNEUMO-001`) e entregavam o diagnóstico na resposta da API.
    """
    label: ExamLabel
    url: str | None = None
    media_type: str = "image"
