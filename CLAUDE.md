...
| `docs/I18N.md` | Plano de internacionalização (mapeamento, decisões D1–D9 fechadas, fases). Fase 0 = `docs/specs/SPEC-007-i18n-fase0.md`. Fases 0, 1 e 1.5 implementadas (10 features + UI compartilhada `app`/`shared`/`core`; `payments`/`minigame`/`questoes` adiadas); **4 idiomas totalmente traduzidos: pt-BR/en/es/ru**; Fase 2 (backend + IA) implementada em duas specs independentes — **SPEC-010 ✅** (códigos de erro estáveis, `app/errors.py`, ~40/138 `HTTPException`) e **SPEC-011 ✅** (idioma nos prompts de IA); catálogo `t("errors.<code>")` no front e long tail de erros ainda pendentes. Ferramental e regras de plural resolvidos: **SPEC-009 ✅** (CSV recortável, merge, header por nome, validações, escrita atômica, colunas `context` e `status:<lang>`) e **SPEC-008 ✅** (paridade por chave-base, `plurals.ts` validando formas via `Intl.PluralRules`, `returnEmptyString: false`). Follow-ups das duas fechados, **menos o TMS**: higiene de chaves órfãs (`i18n:keys`), BCP 47 em `normalizeLocale`, `dirFor` via ICU. `PLURALS_ALLOWLIST` do `parity.test.ts` está vazia — o russo passa nos mesmos testes de plurais que en/es |
| `docs/I18N_IDIOMAS.md` | Referência para **adicionar um idioma novo**: matriz de formas plurais por idioma (CLDR × alcançável na UI), comportamentos verificados do i18next 26 (armadilhas de `count`, fallback silencioso, `_zero`), checklist de 11 passos |
...
npm run i18n:import  # CSV revisado → JSONs de locales (merge; nunca apaga o que não veio no CSV)
npm run i18n:keys    # Chaves órfãs (sem uso) e faltantes (usadas e inexistentes)
# Recorte por rodada (flags após `--`), ver README §i18n:
...
> `backend/tests/` cobre correções de segurança (SPEC-001/002/004/006) e i18n (SPEC-007/010/011) e **roda no CI** desde 2026-07-31 (`ci.yml` → job backend: ruff + py_compile + pytest). A suíte não toca serviço externo: `tests/conftest.py` injeta env dummy, mocka o cliente Supabase **inclusive nas 64 chamadas diretas** (que escapam do `dependency_overrides`, usado por apenas 9 sites) e blinda o cliente da OpenAI. Roda em ~2s — antes levava 3min39s por causa de HTTP real.
> ⚠️ O mock do Supabase (`SupabaseChainMock`) devolve `.data == []`. Um `MagicMock` cru quebraria `classes.create_class`, que faz `while ...execute().data:` para sortear código de turma — com `.data` truthy vira **laço infinito**. Teste que precise de linhas de volta sobrescreve o trecho que lhe importa.
> O frontend tem Vitest (`npm test`), ainda fora do CI.
...
`shared/utils/specialties.ts` is the single source of truth for the 14 medical specialties. Never hardcode specialty lists elsewhere. The `key` field is the canonical value stored in the DB and is **never** translated — to display a specialty use `specialtyLabel(key)`, which resolves `common.specialties.<slug>`.
...
**Backend**
- Keep routes thin: validation + status codes only. All logic goes in `services/`; routes never touch OpenAI/Supabase business logic directly. **Exceção conhecida**: `cases.py::start_ai_chat` chama `get_prompt` + `complete()` direto na rota para a saudação inicial do Chat IA — é o único acesso a IA fora de `services/`. Para mapear prompts, use `grep get_prompt(` em `app/` inteiro, não a convenção de camadas.
