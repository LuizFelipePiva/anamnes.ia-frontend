...
|---|---|---|
| `patient-system-prompt` | Equipe médica | `{{patient_prompt}}`, `{{language}}`, `{{language_name}}` ⚠️ |
| `soap-evaluation-prompt` | Equipe médica | `{{case_summary}}`, `{{soap_content}}`, `{{weight_S}}`, `{{weight_O}}`, `{{weight_A}}`, `{{weight_P}}`, `{{language}}`, `{{language_name}}` ⚠️ |
| `case-generation-prompt` | Equipe médica | `{{description}}`, `{{difficulty}}` (sem `language` — conteúdo gerado é compartilhado e continua pt-BR, SPEC-011 D5) |
| `ai-patient-prompt` | Equipe médica | `{{specialty}}` (pode vir **vazio**, ver abaixo), `{{language}}`, `{{language_name}}` ⚠️ |
...
⚠️ **Passo manual pendente (SPEC-011, 2026-07-31)**: os **três** templates marcados acima recebem `language` (código BCP 47, ex. `ru`) e `language_name` (nome por extenso, ex. `русский`), mas precisam da instrução de idioma no **texto** do template. Enquanto ninguém editar, as variáveis chegam e são ignoradas — o **fallback local** (`_PATIENT_SYSTEM_PROMPT`, `_SOAP_EVALUATION_PROMPT`, `_build_ai_patient_prompt`) já traduz corretamente, então o sintoma é silencioso: só afeta quem tem Langfuse configurado (`LANGFUSE_SECRET_KEY` setado).

**Use `{{language_name}}`, não `{{language}}`** — "responda em `pt-BR`" instrui pior que "responda em `português brasileiro`". As duas variáveis são enviadas nos três call sites, então um template que já use `{{language}}` continua compilando sem erro. Linhas sugeridas (espelham os fallbacks locais):

```
patient-system-prompt →  IDIOMA: Responda sempre em {{language_name}}, independentemente do idioma usado nesta instrução.
soap-evaluation-prompt → IDIOMA: Escreva o campo "feedback" (e os "feedback" do breakdown) sempre em {{language_name}}, independentemente do idioma do SOAP ou do caso clínico.
ai-patient-prompt →      Responda sempre em {{language_name}}, independentemente do idioma usado nesta instrução.
```

O `case-generation-prompt` fica **de fora de propósito** (SPEC-011 §8 E1): caso gerado é conteúdo compartilhado da turma, gravado no banco — segue sempre pt-BR até a Fase 3. Travado por teste.

ℹ️ **`{{specialty}}` do `ai-patient-prompt` pode vir vazio** — o aluno não é obrigado a escolher especialidade ao iniciar o Chat IA. O template de referência abaixo lida bem com isso porque a variável fica **sozinha no fim da frase** (vazia, some sem deixar rastro). Ao reescrever o template, manter essa propriedade: o Langfuse não tem condicional, então algo como "O caso DEVE ser de {{specialty}}." vira "O caso DEVE ser de ." toda vez que o aluno não filtra. (O fallback local `_build_ai_patient_prompt` tem o `if` que o template não pode ter, e por isso alterna entre "O caso DEVE ser de X" e "Varie a especialidade médica…".)
...
**Variáveis:** `{{patient_prompt}}`, `{{language}}`, `{{language_name}}`
...
IDIOMA: Responda sempre em {{language_name}}, independentemente do idioma usado nesta instrução.
```

---

### `soap-evaluation-prompt`
**Variáveis:** `{{case_summary}}`, `{{soap_content}}`, `{{weight_S}}`, `{{weight_O}}`, `{{weight_A}}`, `{{weight_P}}`, `{{language}}`, `{{language_name}}`
...
**Variáveis:** `{{description}}`, `{{difficulty}}` — **sem idioma, de propósito** (SPEC-011 §8 E1: caso gerado é conteúdo compartilhado da turma, segue sempre pt-BR até a Fase 3)
...
### `ai-patient-prompt`
**Variáveis:** `{{specialty}}` (pode vir vazio), `{{language}}`, `{{language_name}}`

> Usado só na **saudação inicial** do Chat IA (`cases.py::start_ai_chat`). Os turnos seguintes usam o `patient-system-prompt`.
