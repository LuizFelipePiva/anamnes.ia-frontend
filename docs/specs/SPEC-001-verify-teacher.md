...
> Nota de implementação: `POST /cases/generate` fica **bloqueado** para student, porém retorna **400** (não 403) por um bug pré-existente do slowapi (`@limiter.limit` consome o corpo antes do guard) — atinge todos os papéis. Bloqueio de segurança OK; correção do 400 fica para SPEC própria.
