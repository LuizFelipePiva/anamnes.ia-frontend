...
import { ChevronRight, Check, X, Search, Play } from 'lucide-react';
import {
  createSimulado,
  fetchFilterOptions,
...
export const SimuladosListPage: React.FC = () => {
  ...
  useEffect(() => {
    fetchFilterOptions().then(data => {
      ...
      const validRequestedSpecialty = requestedSpecialty && data.specialties.includes(requestedSpecialty)
        ? requestedSpecialty
        : null;
      const initialForm: SimuladoCreate = validRequestedSpecialty
      ...
      countAvailableQuestions(initialForm).then(res => {
  ...
  }, [createIntent, requestedSpecialty]);
