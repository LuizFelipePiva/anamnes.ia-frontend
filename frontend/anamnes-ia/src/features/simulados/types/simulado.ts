...
export interface SimuladoCreate {
  title: string;
  description?: string;
  /** Listas para seleção múltipla — vazio = sem filtro */
  specialties?: string[];
  bancas?: string[];
  temas?: string[];
  subtemas?: string[];
...
/** Opções de filtro retornadas pelo backend (valores distintos do banco). */
export interface SimuladoFilterOptions {
  specialties: string[];
  temas_por_especialidade: Record<string, string[]>;
  bancas: string[];
  subtemas: string[];
  anos: number[];
}
